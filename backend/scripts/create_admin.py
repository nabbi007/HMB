...
Usage (from backend/):
    python -m scripts.create_admin --first-name Ama --last-name Admin \
        --phone "+233200000000" --email admin@hmb.app
...
def main() -> None:
    ...
    if pw_err:
        raise SystemExit(pw_err)

    db = SessionLocal()
    try:
        existing = (
            db.query(User).filter((User.phone == args.phone) | (User.email == args.email)).first()
        )
