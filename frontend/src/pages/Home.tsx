  ...
  return (
    <div className="flex h-full flex-col md:block">
      ...
      <div className="flex flex-col gap-4 border-b border-neutral-border bg-background-white px-5 py-4 md:hidden">
        <h1 className="text-2xl leading-tight font-bold text-text-charcoal">
          Care you can <span className="text-brand-red">trust</span>, near you.
        </h1>
        <SearchBar
          search={search}
      ...
      <div className="relative flex-1 md:absolute md:inset-0">
        {MAPBOX_TOKEN ? (
          ...
          >
            ...
            {filtered.map((c) => (
              <CaregiverMapMarker key={c.id} caregiver={c} highlighted={hoveredId === c.id} />
            ))}
          </Map>
        ) : (
          <MapTokenNotice />
        ...
        <div className="absolute top-6 left-12 hidden h-[calc(100%-48px)] w-[440px] flex-col gap-5 rounded-card bg-background-white p-7 shadow-[0_16px_40px_-8px_rgba(0,0,0,0.16)] md:flex">
          ...
          <div className="-mx-2 -mt-1 min-h-0 flex-1 overflow-y-auto px-2">
            {filtered.map((c) => (
        ...
        <div className="absolute inset-x-0 bottom-0 flex max-h-[55%] flex-col gap-3 rounded-t-card bg-background-white px-5 pt-3 pb-4 shadow-[0_-8px_24px_-8px_rgba(0,0,0,0.16)] md:hidden">
          <span className="mx-auto h-1 w-10 rounded-full bg-neutral-border" />
          <p className="text-sm font-medium text-text-muted">
            {filtered.length} caregiver{filtered.length === 1 ? "" : "s"} near you
          </p>
